...
const isArrayLikeOptions = (options)=>{
    return typeof options === 'object' && options !== null && 'length' in options;
};
const isMultipleSelectTarget = (target)=>{
 ...
 */ const Form = /*#__PURE__*/ React__namespace.forwardRef(({ disabled = false, method, onSubmit, initialErrors, ...props }, ref)=>{
    ...
    const modified = React__namespace.useMemo(()=>!isEqual__default.default(initialValues.current, state.values), // eslint-disable-next-line react-hooks/exhaustive-deps
    [
        state.values,
...
const setDraftValues = (draft, values)=>{
    draft.values = values;
};
const reducer = (state, action)=>immer.produce(state, (draft)=>{
        switch(action.type){
            ...
            case 'SET_FIELD_VALUE':
                setDraftValues(draft, objects.setIn(state.values, action.payload.field, action.payload.value));
                break;
            case 'ADD_FIELD_ROW':
                {
                    /**
         ...
         */ const currentField = objects.getIn(state.values, action.payload.field, []);
                    ...
                    const existingKeys = new Set(currentField.map((item)=>item.__temp_key__).filter((key)=>key !== undefined && key !== ''));
                    ...
                    setDraftValues(draft, objects.setIn(state.values, action.payload.field, currentField.toSpliced(position, 0, {
         ...
         */ const currentField = [
                    ...
                    setDraftValues(draft, objects.setIn(state.values, field, currentField));
                    break;
                }
         ...
         */ const newValue = objects.setIn(currentField, position.toString(), undefined).filter((val)=>val);
                    setDraftValues(draft, objects.setIn(state.values, action.payload.field, newValue.length > 0 ? newValue : []));
                    break;
                }
            case 'SET_ERRORS':
