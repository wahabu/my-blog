...
var relations = {
   ...
   */ async findAvailable (ctx) {
        ...
        const { idsToOmit, idsToInclude, // `_q` is the public query parameter name; keep the alias grep-able.
        _q: search, ...query } = ctx.request.query;
        const permissionChecker = index.getService('permission-checker').create({
     ...
     */ if (search) {
            const filter = strapiUtils.isOperatorOfType('where', query._filter) ? query._filter : '$containsi';
            addFiltersClause(queryParams, {
                [mainField]: {
                    [filter]: search
                }
