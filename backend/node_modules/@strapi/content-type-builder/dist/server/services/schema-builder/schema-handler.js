...
function createSchemaHandler(infos) {
    ...
    return {
        ...
        get (path) {
            return ___default.default.get(state.schema, path);
        },
        // set a particular path inside the schema
        set (path, val) {
            if (!state.schema) return this;
            modified = true;
            const value = ___default.default.defaultTo(val, ___default.default.get(state.schema, path));
            ___default.default.set(state.schema, path, value);
            return this;
        },
        // delete a particular path inside the schema
        unset (path) {
            modified = true;
            ___default.default.unset(state.schema, path);
            return this;
        },
        delete () {
            deleted = true;
        ...
        setAttributes (newAttributes) {
            ...
            for(const key in this.schema.attributes){
                if (attributes.isConfigurable(this.schema.attributes[key])) {
        ...
        async flush () {
            ...
            const initialPath = path__default.default.join(initialState.dir, initialState.filename);
            const filePath = path__default.default.join(state.dir, state.filename);
            if (deleted) {
                await fse__default.default.remove(initialPath);
                const list = await fse__default.default.readdir(initialState.dir);
                if (list.length === 0) {
                    await fse__default.default.remove(initialState.dir);
                }
                return;
            }
            if (modified) {
                ...
                await fse__default.default.writeJSON(filePath, {
        ...
        async rollback () {
            ...
            if (!initialState.uid) {
                await fse__default.default.remove(filePath);
                const list = await fse__default.default.readdir(state.dir);
                if (list.length === 0) {
                    await fse__default.default.remove(state.dir);
                }
                return;
            }
            if (modified || deleted) {
                await fse__default.default.ensureFile(initialPath);
                await fse__default.default.writeJSON(initialPath, initialState.schema, {
                    spaces: 2
                });
                // remove
                if (initialPath !== filePath) {
