...
import { permission } from './common-validators.mjs';

const adminTokenCreationSchema = yup.object().shape({
...
const adminTokenUpdateSchema = yup.object().shape({
    name: yup.string().min(1).notNull(),
    description: yup.string().nullable(),
    adminPermissions: yup.array().of(permission).nullable()
}).noUnknown().strict();
const validateAdminTokenCreationInput = validateYupSchema(adminTokenCreationSchema);
const validateAdminTokenUpdateInput = validateYupSchema(adminTokenUpdateSchema);

export { validateAdminTokenCreationInput, validateAdminTokenUpdateInput };
//# sourceMappingURL=admin-tokens.mjs.map
