 ...
 */ const resolveCornerResize = ({ anchorX, anchorY, point, aspectRatio })=>{
 ...
 */ const useCropImg = ()=>{
   ...
   */ const init = React.useCallback((image)=>{
    ...
    /** Update crop dimensions, clamping to remain inside the image bounds. */ const setCropSize = React.useCallback((next)=>{
        setCrop((prev)=>{
    ...
    /** Update crop origin, clamping so the crop stays inside the image. */ const setCropPosition = React.useCallback((next)=>{
        setCrop((prev)=>{
            const x = next.x !== undefined ? clamp(next.x, 0, naturalSize.width - prev.width) : prev.x;
            const y = next.y !== undefined ? clamp(next.y, 0, naturalSize.height - prev.height) : prev.y;
            return {
                ...prev,
                x,
                y
    ...
    /** Lock or release the crop aspect ratio. Pass `null` to free it. */ const setAspectRatio = React.useCallback((ratio)=>{
...
export { resolveCornerResize, useCropImg };
