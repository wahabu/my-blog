    ...
    constructor(){
        this.register = (customFields)=>{
            if (Array.isArray(customFields)) {
                ...
                invariant__default.default(ALLOWED_TYPES.includes(type), `Custom field type: '${type}' is not a valid Strapi type or it can't be used with a Custom Field`);
                // Ensure name has no special characters
                const isValidObjectKey = /^(?![0-9])[a-zA-Z0-9$_-]+$/g;
                invariant__default.default(isValidObjectKey.test(name), `Custom field name: '${name}' is not a valid object key`);
