...
var createValidateHelpers = (({ action, ability: ability$1, model })=>{
    ...
    const createValidateQuery = (options = {})=>{
        ...
        const createValidateFilters = (ctx)=>utils.async.pipe(utils.traverse.traverseQueryFilters(throwDisallowedFields(permittedFields), ctx), utils.traverse.traverseQueryFilters(throwDisallowedAdminUserFields, ctx), utils.traverse.traverseQueryFilters(throwPassword, ctx), utils.traverse.traverseQueryFilters(({ key, value, path })=>{
        ...
        const createValidateSort = (ctx)=>utils.async.pipe(utils.traverse.traverseQuerySort(throwDisallowedFields(permittedFields), ctx), utils.traverse.traverseQuerySort(throwDisallowedAdminUserFields, ctx), utils.traverse.traverseQuerySort(throwPassword, ctx), utils.traverse.traverseQuerySort(({ key, attribute, value, path })=>{
        ...
        const createValidateFields = (ctx)=>utils.async.pipe(utils.traverse.traverseQueryFields(throwDisallowedFields(permittedFields), ctx), utils.traverse.traverseQueryFields(throwPassword, ctx));
