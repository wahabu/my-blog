...
const FormModal = ()=>{
    ...
    const checkFormValidity = async ()=>{
        ...
        // This is happening when the user click on the link from the left menu
            ...
            schema = forms.component.schema(Object.keys(components), toStringValue(modifiedData.category), reservedNames, actionType === 'edit', components, toStringValue(modifiedData.displayName), type?.uid ?? null);
        ...
        // The data is set in the componentToCreate key
            ...
            schema = forms.component.schema(Object.keys(components), toStringValue(get(modifiedData, 'componentToCreate.category', '')), reservedNames, actionType === 'edit', components, toStringValue(modifiedData.componentToCreate?.displayName));
        ...
        // We need to make sure that it is independent from the step
            ...
            if (computedAttributeType === 'relation') {
                const targetContentTypeUID = toStringValue(get(modifiedData, [
                    'target'
                ], ''));
            ...
            schema = forms.attribute.schema(type, computedAttributeType ?? 'string', reservedNames, alreadyTakenTargetContentTypeAttributes, {
                modifiedData,
                initialData
            }, ctbFormsAPI);
        } else {
        ...
        try {
            ...
            // We are creating a component using the component modal from the left menu
                ...
                if (isCreating) {
                    ...
                    if (type.status === 'NEW') {
                        const componentUid = createComponentUid(toStringValue(modifiedData.displayName), toStringValue(modifiedData.category));
                        updateComponentUid({
                            componentUID: targetUid,
            ...
            // We need to prevent the component from being created if the user closes the modal at step 2 without any submission
                ...
                const componentUid = createComponentUid(draftComponent.displayName, draftComponent.category);
