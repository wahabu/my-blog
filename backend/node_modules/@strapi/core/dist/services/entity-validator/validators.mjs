 ...
 */ const addMaxLengthValidator = (validator, { attr })=>{
    ...
    if (attr.type === 'string') {
        const max = _.isInteger(attr.maxLength) ? attr.maxLength : 255;
...
const floatValidator = (metas, options)=>{
    let schema = yup.number().test('is-finite-number', // eslint-disable-next-line no-template-curly-in-string -- Yup interpolates validation path placeholders.
...
const bigintegerValidator = (metas, options)=>{
    const schema = yup.mixed().transform((value, originalValue)=>toBigIntegerString(originalValue) ?? value)// eslint-disable-next-line no-template-curly-in-string -- Yup interpolates validation path placeholders.
