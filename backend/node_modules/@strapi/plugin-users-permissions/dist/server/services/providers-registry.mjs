...
import { __require as requireVerifyJwtWithJwks } from '../utils/verify-jwt-with-jwks.mjs';
import { __require as requireProviderHttp } from '../utils/provider-http.mjs';
import { __require as requireOauth1 } from '../utils/oauth-connect/oauth1.mjs';
...
function requireProvidersRegistry() {
    ...
    const { verifyJwtWithJwks } = requireVerifyJwtWithJwks();
    const { bearerGet, fetchJson } = requireProviderHttp();
    const initProviders = ({ baseURL })=>({
            ...
            google: {
                ...
                async authCallback ({ accessToken }) {
                    const { body } = await fetchJson(`https://oauth2.googleapis.com/tokeninfo?access_token=${encodeURIComponent(accessToken)}`);
                    return {
            ...
            github: {
                ...
                async authCallback ({ accessToken }) {
                    ...
                    const { body: emailBody } = await bearerGet('https://api.github.com/user/emails', accessToken, {
                        headers: {
                            'user-agent': 'strapi'
                        }
                    });
            ...
            twitter: {
                ...
                async authCallback ({ accessToken, query, providers }) {
                    const { twitterGet } = requireOauth1();
            ...
            linkedin: {
                ...
                async authCallback ({ accessToken }) {
                    ...
                    const { body: emailBody } = await bearerGet('https://api.linkedin.com/v2/emailAddress?q=members&projection=(elements*(handle~))', accessToken);
            ...
            cognito: {
                ...
                async authCallback ({ query, providers }) {
                    ...
                    const tokenPayload = await verifyJwtWithJwks({
                        idToken,
                        jwksUrl
                    });
                    return {
            ...
            auth0: {
                ...
                async authCallback ({ accessToken, providers }) {
                    const { body } = await bearerGet(`https://${providers.auth0.subdomain}.auth0.com/userinfo`, accessToken);
            ...
            cas: {
                ...
                async authCallback ({ accessToken, providers }) {
                    const { body } = await bearerGet(`https://${providers.cas.subdomain}/oidc/profile`, accessToken);
            ...
            patreon: {
                ...
                async authCallback ({ accessToken }) {
                    const { body } = await bearerGet('https://www.patreon.com/api/oauth2/v2/identity?fields[user]=full_name,email', accessToken);
            ...
            keycloak: {
                ...
                async authCallback ({ accessToken, providers }) {
                    const { body } = await bearerGet(`https://${providers.keycloak.subdomain}/protocol/openid-connect/userinfo`, accessToken);
                    return {
