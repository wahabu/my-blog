...
const readPackageJSON = async (path)=>{
    try {
        const packageObj = await fse__default.default.readJson(path);
        return packageObj;
    } catch (err) {
...
const writePackageJSON = async (path, file, spacing)=>{
    try {
        await fse__default.default.writeJson(path, file, {
            spaces: spacing
        });
        return true;
    } catch (err) {
        if (err instanceof Error) {
            console.error(`${chalk__default.default.red('Error')}: ${err.message}`);
            console.log(`${chalk__default.default.yellow('Warning')}: There has been an error, please set "telemetryDisabled": false in the "strapi" object of your package.json manually.`);
            return false;
        }
        throw err;
    }
