...
var draftRelations = require('../services/utils/draft-relations.js');
    ...
    async countDraftRelations (ctx) {
        ...
        if (permissionChecker.requiresEntity.read()) {
            ...
            if (!entity) {
                ...
                const versions = await documentManager.findLocales(id, model, {
                    populate
                });
                if (versions.length === 0) {
                    return ctx.notFound();
                }
                if (permissionChecker.requiresEntity.read() && versions.every((version)=>permissionChecker.cannot.read(version))) {
                    return ctx.forbidden();
                }
                return {
                    data: draftRelations.EMPTY_DRAFT_RELATION_COUNTS
                };
            }
