        ...
        async createMany (stagesList, { fields } = {}) {
            ...
            await async.reduce(stagesList)(async (_, stage, idx)=>{
                ...
                if (stage.fromPermissions && stage.fromPermissions.length > 0) {
                    const fromPermissions = await async.map(stage.fromPermissions, (permission)=>stagePermissionsService.register({
                ...
                if (stage.toPermissions && stage.toPermissions.length > 0) {
                    const toPermissions = await async.map(stage.toPermissions, (permission)=>stagePermissionsService.registerTo({
                            roleId: permission.role,
                            action: permission.action,
                            toStage: stageId
                        }));
        ...
        async update (srcStage, destStage) {
            ...
            if (hasPermissionChanges) {
                ...
                if (fromPerms.length > 0) {
                    const registered = await async.map(fromPerms, (permission)=>stagePermissionsService.register({
                            roleId: permission.role,
                            action: permission.action,
                            fromStage: stageId
                        }));
                ...
                if (toPerms.length > 0) {
                    const registered = await async.map(toPerms, (permission)=>stagePermissionsService.registerTo({
                            roleId: permission.role,
                            action: permission.action,
                            toStage: stageId
                        }));
