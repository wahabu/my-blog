 ...
 */ const NO_FILES = [];
const selectCompletedUploads = createSelector(// The plugin registers this slice in `register()`, so a host that has not
// reached that point has no upload state — and therefore nothing to place.
(state)=>state.uploadProgress?.files ?? NO_FILES, (files)=>files.reduce((completed, row)=>{
 ...
 */ const selectReportsByteProgress = createSelector((state)=>state.uploadProgress.reportsByteProgress, (state)=>state.uploadProgress.files, (reportsByteProgress, files)=>reportsByteProgress || files.some((f)=>f.status === 'uploading' && f.uploadedBytes > 0));
/**
 ...
 */ const selectCountBasedProgress = createSelector((state)=>state.uploadProgress.files, (files)=>{
    const settled = files.filter((f)=>f.status === 'complete' || f.status === 'error' || f.status === 'cancelled').length;
...
const { openUploadProgress, appendUploadFiles, setFileUploading, setFileProgress, setFileComplete, setFileError, setFileMetadataGenerating, setFileMetadataResult, addUploadErrors, closeUploadProgress, toggleMinimize, cancelUpload, setUploadFailed, retryCancelledFiles } = uploadProgressSlice.actions;
/**
 ...
 */ const isUploadInFlight = (files)=>files.some((f)=>f.status === 'pending' || f.status === 'uploading');
const uploadProgressReducer = uploadProgressSlice.reducer;

export { addUploadErrors, appendUploadFiles, cancelUpload, closeUploadProgress, isUploadInFlight, openUploadProgress, retryCancelledFiles, selectAggregateProgress, selectCompletedUploads, selectCountBasedProgress, selectIsGeneratingMetadata, selectMetadataOutcome, selectMetadataProgress, selectReportsByteProgress, setFileComplete, setFileError, setFileMetadataGenerating, setFileMetadataResult, setFileProgress, setFileUploading, setUploadFailed, toggleMinimize, uploadProgressReducer };
