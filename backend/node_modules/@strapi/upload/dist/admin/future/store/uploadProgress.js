 ...
 */ const NO_FILES = [];
const selectCompletedUploads = toolkit.createSelector(// The plugin registers this slice in `register()`, so a host that has not
 ...
 */ const selectReportsByteProgress = toolkit.createSelector((state)=>state.uploadProgress.reportsByteProgress, (state)=>state.uploadProgress.files, (reportsByteProgress, files)=>reportsByteProgress || files.some((f)=>f.status === 'uploading' && f.uploadedBytes > 0));
/**
 ...
 */ const selectCountBasedProgress = toolkit.createSelector((state)=>state.uploadProgress.files, (files)=>{
    const settled = files.filter((f)=>f.status === 'complete' || f.status === 'error' || f.status === 'cancelled').length;
...
exports.appendUploadFiles = appendUploadFiles;
exports.cancelUpload = cancelUpload;
exports.closeUploadProgress = closeUploadProgress;
exports.isUploadInFlight = isUploadInFlight;
...
exports.selectCompletedUploads = selectCompletedUploads;
exports.selectCountBasedProgress = selectCountBasedProgress;
...
exports.selectReportsByteProgress = selectReportsByteProgress;
