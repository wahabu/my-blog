 ...
 */ const createEntitiesStream = (strapi, options = {})=>{
    ...
    return stream.Readable.from(async function* entitiesGenerator() {
        for await (const { stream, contentType } of contentTypeStreamGenerator()){
            try {
                ...
                options.onWarning?.(`Failed to read all entities of type "${contentType.uid}" from the source, the remaining entities of this type were skipped: ${message}`);
            } finally{
                stream.destroy();
            }
        }
