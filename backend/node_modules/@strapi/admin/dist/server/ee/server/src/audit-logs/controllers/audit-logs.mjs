...
import { AUDIT_LOGS_EXPORT_UNTIL_HEADER, AUDIT_LOGS_EXPORT_PART_SIZE_HEADER, AUDIT_LOGS_EXPORT_TOKEN_HEADER, AUDIT_LOGS_EXPORT_NEXT_CURSOR_HEADER, AUDIT_LOGS_EXPORT_CURSOR_DONE } from '../../../../../shared/utils/audit-log-export.mjs';
import { validateExport, validateFindManyUsers, validateFindMany } from '../validation/audit-logs.mjs';
...
var auditLogsController = {
    ...
    async findManyUsers (ctx) {
        ...
        const auditLogs = strapi.get('audit-logs');
        const body = await auditLogs.findManyUsers({
            page,
            pageSize
        });
        ctx.body = body;
    ...
    async export (ctx) {
        ...
        if (!isContinuation) {
            if (await auditLogs.isExportTooLarge({
                filters: validatedQuery.filters ?? null
            })) {
                throw new errors.PayloadTooLargeError(EXPORT_TOO_LARGE_MESSAGE);
            }
        }
        const { stream, nextCursor, until, isNewExport, partSize, exportToken } = await auditLogs.createExportStream(validatedQuery);
        ...
        ctx.set(AUDIT_LOGS_EXPORT_NEXT_CURSOR_HEADER, nextCursor !== null ? String(nextCursor) : AUDIT_LOGS_EXPORT_CURSOR_DONE);
