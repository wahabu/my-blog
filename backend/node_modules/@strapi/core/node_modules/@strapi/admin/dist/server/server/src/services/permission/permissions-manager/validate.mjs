...
var createValidateHelpers = (({ action, ability, model })=>{
    ...
    const createValidateQuery = (options = {})=>{
        ...
        const createValidateFilters = (ctx)=>async.pipe(traverse.traverseQueryFilters(throwDisallowedFields(permittedFields), ctx), traverse.traverseQueryFilters(throwDisallowedAdminUserFields, ctx), traverse.traverseQueryFilters(throwPassword, ctx), traverse.traverseQueryFilters(({ key, value, path })=>{
        ...
        const createValidateSort = (ctx)=>async.pipe(traverse.traverseQuerySort(throwDisallowedFields(permittedFields), ctx), traverse.traverseQuerySort(throwDisallowedAdminUserFields, ctx), traverse.traverseQuerySort(throwPassword, ctx), traverse.traverseQuerySort(({ key, attribute, value, path })=>{
        ...
        const createValidateFields = (ctx)=>async.pipe(traverse.traverseQueryFields(throwDisallowedFields(permittedFields), ctx), traverse.traverseQueryFields(throwPassword, ctx));
        ...
        const validateNestedPopulate = async ({ key, value, schema, attribute, getModel })=>{
            if (attribute) {
                return;
            }
            const nestedCtx = {
        ...
        const validatePopulate = async.pipe(traverse.traverseQueryPopulate(throwDisallowedFields(permittedFields), ctx), traverse.traverseQueryPopulate(throwDisallowedAdminUserFields, ctx), traverse.traverseQueryPopulate(throwHiddenFields, ctx), traverse.traverseQueryPopulate(throwPassword, ctx), traverse.traverseQueryPopulate(validateNestedPopulate, ctx));
        return async (query)=>{
            if (query.filters) {
...
export { createValidateHelpers as default };
