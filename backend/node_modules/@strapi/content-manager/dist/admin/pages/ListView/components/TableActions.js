...
`;
const OpenInNewTabAction = ({ documentId })=>{
    const { formatMessage } = reactIntl.useIntl();
    const { canRead } = DocumentRBAC.useDocumentRBAC('OpenInNewTabAction', ({ canRead })=>({
            canRead
        }));
    const [{ query }] = strapiAdmin.useQueryParams();
    ...
    const href = reactRouterDom.useHref({
        pathname: documentId ?? '',
        search: qs.stringify({
            plugins: query.plugins
        })
    ...
    return {
        ...
        icon: /*#__PURE__*/ jsxRuntime.jsx(StyledExternalLink, {}),
        label: formatMessage({
...
const StyledExternalLink = styledComponents.styled(Icons.ExternalLink)`
  path {
    fill: currentColor;
  }
`;
