...
import { escapeQuery } from './search.mjs';
...
const applyOperator = (qb, column, operator, value)=>{
    ...
    switch(operator){
        ...
        case '$eqi':
            {
                ...
                qb.whereRaw(`${fieldLowerFn(qb)} = LOWER(?)`, [
                    column,
                    `${value}`
                ]);
                break;
        ...
        case '$startsWith':
            {
                qb.whereRaw(`?? LIKE ?${likeEscapeClause(qb)}`, [
                    column,
                    `${escapeLike(value)}%`
                ]);
                break;
        ...
        case '$notContains':
            {
                qb.whereRaw(`?? NOT LIKE ?${likeEscapeClause(qb)}`, [
                    column,
                    `%${escapeLike(value)}%`
                ]);
                break;
...
const escapeLike = (value)=>escapeQuery(`${value}`, LIKE_SPECIAL_CHARS);
...
const likeEscapeClause = (qb)=>qb.client.dialect === 'sqlite3' ? " ESCAPE '\\'" : '';
