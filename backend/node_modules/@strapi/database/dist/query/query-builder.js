...
const createQueryBuilder = (uid, db, initialState = {})=>{
    ...
    return {
        ...
        init (params = {}) {
            const { _q: searchQuery, filters, where, select, limit, offset, orderBy, groupBy, populate } = params;
            if (!___default.default.isNil(where)) {
                this.where(where);
            }
            if (!___default.default.isNil(searchQuery)) {
     ...
     */ ensurePaginationOrderStability () {
        ...
        processSelect () {
            ...
            if (this.shouldUseDistinct()) {
                ...
                const rawOrderByExpressions = state.orderBy.filter((ob)=>'rawExpression' in ob).map((ob)=>orderBy.buildStatusSortExpression(db, tableName, this.alias, ob.isI18n));
                state.select = [
                    ...state.select,
                    ...rawOrderByExpressions
                ];
            }
        },
        getKnexQuery () {
            ...
            switch(state.type){
                ...
                case 'min':
                    {
                        const dbColumnName = this.aliasColumn(transform.toColumnName(meta, state.min));
                        qb.min({
                            min: dbColumnName
                        });
                        break;
        ...
        async execute ({ mapResults = true } = {}) {
            try {
                ...
                if (state.populate && !___default.default.isNil(rows)) {
                    await apply(___default.default.castArray(rows), state.populate, {
                        qb: this,
                        uid,
                        db
                    });
