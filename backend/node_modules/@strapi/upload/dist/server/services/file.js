...
const { bytesToHumanReadable } = utils.file;
 ...
 */ const fetchUrlToInputFile = async (url, tmpWorkingDirectory, sizeLimit, onProgress)=>{
    ...
    const tooLargeError = ()=>new ApplicationError(`File too large: maximum allowed size is ${bytesToHumanReadable(sizeLimit ?? 0)}`);
    ...
    const reportProgress = (progress)=>{
        ...
        try {
            ...
            strapi?.log?.warn(`URL fetch progress callback threw, ignoring: ${error instanceof Error ? error.message : error}`);
        }
    };
    ...
    try {
        if (response.body === null) {
            ...
            await fse__default.default.writeFile(tmpFilePath, '');
        } else {
            const counter = new node_stream.Transform({
            ...
            await promises.pipeline(node_stream.Readable.fromWeb(response.body), counter, node_fs.createWriteStream(tmpFilePath));
        }
    } catch (error) {
        ...
        try {
            await fse__default.default.remove(tmpFilePath);
        } catch (cleanupError) {
            strapi?.log?.warn(`Could not remove partial temp file ${tmpFilePath}: ${cleanupError instanceof Error ? cleanupError.message : cleanupError}`);
        }
        throw error;
    }
    // Derive the size from the file on disk rather than from a buffer we never hold
    const { size } = await fse__default.default.stat(tmpFilePath);
