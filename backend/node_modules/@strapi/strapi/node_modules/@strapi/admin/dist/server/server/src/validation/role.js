...
const rolesDeleteSchema = utils.yup.object().shape({
    ids: utils.yup.array().of(utils.yup.strapiID()).min(1).required().test('roles-deletion-checks', 'Roles deletion checks have failed', async function checkRolesDeletion(ids) {
        try {
...
const roleDeleteSchema = utils.yup.strapiID().required().test('no-admin-single-delete', 'Role deletion checks have failed', async function checkRoleDeletion(id) {
    try {
