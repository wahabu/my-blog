...
require('events');
require('ws');
var transferPolicy = require('../../transfer-policy.js');
...
const reportTeardownError = (error)=>{
    strapi?.log?.error('[Data transfer] Failed to clean up push transfer');
    strapi?.log?.error(error);
};
const abortPushTransfer = async ({ provider, streams, assets, cleanup })=>{
    try {
...
const writeValidatedPushStreamBatch = async (strapi1, stage, data, stream, stats)=>{
    if (stage === 'entities') {
        for (const entity of data){
            transferPolicy.assertRemoteEntityAllowed(strapi1, entity);
        }
    } else {
        for (const link of data){
            transferPolicy.assertRemoteLinkAllowed(link);
        }
    }
...
const createPushController = utils.handlerControllerFactory((proto)=>({
        ...
        async abort (terminal = false) {
            ...
            this.abortPromise = abortPushTransfer({
                ...this,
        ...
        async processMessage (raw) {
            ...
            if (this.terminal) {
                await this.respond(msg.uuid, new providers.ProviderTransferError('Transfer has been terminated'));
                return;
            }
            ...
            if (msg.action === 'stream') {
                ...
                try {
                    if (stage === 'entities' || stage === 'links') {
                        await writeValidatedPushStreamBatch(strapi, stage, msg.data, stream, this.stats[stage]);
                        return;
                    }
                } catch (error) {
        ...
        async init (params) {
            ...
            this.provider = index$1.createLocalStrapiDestinationProvider({
                ...
                restore: transferPolicy.normalizeRemoteRestoreOptions(strapi, params?.options?.restore ?? {}),
                autoDestroy: false,
                getStrapi: ()=>strapi
            });
...
exports.abortPushTransfer = abortPushTransfer;
exports.createPushController = createPushController;
exports.writeValidatedPushStreamBatch = writeValidatedPushStreamBatch;
