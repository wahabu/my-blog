...
var i18n = require('./relations/utils/i18n.js');
 ...
 */ const createIdMap = ({ strapi: strapi1 })=>{
     ...
     */ async load () {
            ...
            await strapiUtils.async.map(Object.values(idsByUidAndLocale), async ({ uid, locale, documentIds, status })=>{
                ...
                result?.forEach(({ documentId, id, locale: entryLocale, publishedAt })=>{
                    const key = encodeKey({
                        documentId,
                        uid,
                        locale: i18n.isLocalizedContentType(uid) ? entryLocale : null,
