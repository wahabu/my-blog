    ...
    const { selectedEnvironment } = await inquirer.prompt([
        {
            ...
            choices: [
                ...choices,
                {
                    name: chalk__default.default.grey(`(${QUIT_OPTION})`),
                    value: null
                }
            ]
        }
...
async function upload(ctx, project, cliConfig, token, maxProjectFileSize) {
    ...
    try {
        ...
        const projectFolder = path__default.default.resolve(process.cwd());
        ...
        const tarFilePath = path__default.default.resolve(storagePath, compressedFilename);
        ...
        try {
            ...
            progressBar.stop();
            ctx.logger.log(`${chalk__default.default.green.bold('✔')} Upload finished!\n`);
...
var action = (async (ctx, opts)=>{
    ...
    try {
        ...
        if (e instanceof axios.AxiosError && e.response?.data) {
            if (e.response.status === 404) {
                ctx.logger.warn(`The project associated with this folder does not exist in Strapi Cloud. \nPlease link your local project to an existing Strapi Cloud project using the ${chalk__default.default.cyan('link')} command before deploying.`);
            } else {
    ...
    if (!opts.force) {
        ...
        if (shouldDisplayWarning) {
            ctx.logger.log(boxen__default.default(cliConfig.projectDeployment.confirmationText, boxenOptions));
            const { default: inquirer } = await import('inquirer');
    ...
    try {
        ctx.logger.log(`∷ Deploying project to ${chalk__default.default.cyan(project.targetEnvironment ?? `production`)} environment...`);
        ...
        const dashboardUrlLine = chalk__default.default.cyan(' →  ') + chalk__default.default.cyan.underline(`${api.apiConfig.dashboardBaseUrl}/projects/${project.name}/deployments`);
        ctx.logger.log(boxen__default.default(`Project and deployment logs ready at:\n${dashboardUrlLine}`, {
            padding: 1,
            margin: 1,
            borderStyle: 'round',
            borderColor: 'white',
