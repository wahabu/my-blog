...
const getFilePath = (directoryPath)=>path__default.default.join(directoryPath || process.cwd(), LOCAL_SAVE_FILENAME);
async function save(data, { directoryPath } = {}) {
    ...
    await fse__default.default.ensureDir(path__default.default.dirname(pathToFile));
    await fse__default.default.writeJson(pathToFile, data, {
        encoding: 'utf8'
    });
}
async function retrieve({ directoryPath } = {}) {
    ...
    return fse__default.default.readJSON(pathToFile, {
        encoding: 'utf8'
    });
}
async function patch(patchData, { directoryPath } = {}) {
    ...
    await fse__default.default.writeJson(pathToFile, newData, {
        encoding: 'utf8'
    });
}
async function deleteConfig({ directoryPath } = {}) {
    ...
    const pathExists = await fse__default.default.pathExists(pathToFile);
    if (pathExists) {
        await fse__default.default.remove(pathToFile);
    }
}
