        ...
        case 'uid':
            {
                return {
                    required: common.validators.required,
                    targetField: utils.yup.string().oneOf(Object.keys(attributes).filter((key)=>constants.VALID_UID_TARGETS.includes(___default.default.get(attributes[key], 'type')))).nullable(),
                    default: utils.yup.string().test('isValidDefaultUID', 'cannot define a default UID if the targetField is set', function isValidDefaultUID(value) {
                        const { targetField } = this.parent;
                        return !!(___default.default.isNil(targetField) || ___default.default.isNil(value));
                    }).test('isValidDefaultRegexUID', `\${path} must match the custom regex or the default one "${common.UID_REGEX}"`, function isValidDefaultRegexUID(value) {
                        const { regex } = this.parent;
                        if (regex) {
                            return !___default.default.isNil(value) && (value === '' || new RegExp(regex).test(value));
                        }
        ...
        case 'dynamiczone':
            {
                return {
                    required: common.validators.required,
                    components: utils.yup.array().of(utils.yup.string().required()).test('isArray', // eslint-disable-next-line no-template-curly-in-string -- Yup interpolation placeholder
                    '${path} must be an array', (value)=>Array.isArray(value)).min(1),
                    min: utils.yup.number(),
                    max: utils.yup.number()
                };
            }
