   ...
   */ get cleanupThreshold() {
    ...
    async rotateRefreshToken(refreshToken, origin) {
        ...
        try {
            const config = this.getConfigForOrigin(origin);
            const algorithm = config.algorithm || constants.DEFAULT_ALGORITHM;
            const verifyKey = this.getJwtKey(config, algorithm, 'verify');
            const signKey = this.getJwtKey(config, algorithm, 'sign');
            // Filter out conflicting options that are already handled by the payload or key selection
            const jwtOptions = config.jwtOptions || {};
            const { expiresIn: _expiresIn, privateKey: _privateKey, publicKey: _publicKey, ...jwtSignOptions } = jwtOptions;
            const payload = jwt__default.default.verify(refreshToken, verifyKey, {
                algorithms: [
                    algorithm
                ],
                ...jwtSignOptions
            ...
            if (current.childId) {
                ...
                if (child) {
                    ...
                    const childToken = jwt__default.default.sign(childPayload, signKey, {
                        algorithm,
                        noTimestamp: true,
                        ...jwtSignOptions
                    });
            ...
            const childToken = jwt__default.default.sign(payloadOut, signKey, {
                algorithm,
                noTimestamp: true,
                ...jwtSignOptions
            });
