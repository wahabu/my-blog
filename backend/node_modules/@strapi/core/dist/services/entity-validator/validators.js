 ...
 */ const addMaxLengthValidator = (validator, { attr })=>{
    ...
    if (attr.type === 'string') {
        const max = ___default.default.isInteger(attr.maxLength) ? attr.maxLength : 255;
...
const floatValidator = (metas, options)=>{
    let schema = strapiUtils.yup.number().test('is-finite-number', // eslint-disable-next-line no-template-curly-in-string -- Yup interpolates validation path placeholders.
    '${path} must be a finite number', (value)=>value == null || isValidFiniteNumber(value));
...
const bigintegerValidator = (metas, options)=>{
    const schema = strapiUtils.yup.mixed().transform((value, originalValue)=>toBigIntegerString(originalValue) ?? value)// eslint-disable-next-line no-template-curly-in-string -- Yup interpolates validation path placeholders.
