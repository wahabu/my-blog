...
import 'events';
import 'ws';
import { normalizeRemoteRestoreOptions, assertRemoteEntityAllowed, assertRemoteLinkAllowed } from '../../transfer-policy.mjs';
...
const writeValidatedPushStreamBatch = async (strapi1, stage, data, stream, stats)=>{
    if (stage === 'entities') {
        for (const entity of data){
            assertRemoteEntityAllowed(strapi1, entity);
        }
    } else {
...
const createPushController = handlerControllerFactory((proto)=>({
        ...
        async init (params) {
            ...
            this.provider = createLocalStrapiDestinationProvider({
                ...
                restore: normalizeRemoteRestoreOptions(strapi, params?.options?.restore ?? {}),
                autoDestroy: false,
                getStrapi: ()=>strapi
            });
...
export { abortPushTransfer, createPushController, writeValidatedPushStreamBatch };
