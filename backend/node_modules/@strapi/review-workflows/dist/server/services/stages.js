...
var stages = (({ strapi })=>{
    ...
    return {
        find ({ workflowId, populate }) {
            return strapi.db.query(workflows.STAGE_MODEL_UID).findMany({
                where: {
                    workflow: workflowId
                },
                populate
        ...
        findById (id, { populate } = {}) {
            return strapi.db.query(workflows.STAGE_MODEL_UID).findOne({
                where: {
                    id
                },
                populate
        ...
        async createMany (stagesList, { fields } = {}) {
            ...
            const stages = await Promise.all(stagesList.map((stage)=>strapi.db.query(workflows.STAGE_MODEL_UID).create({
            ...
            await utils.async.reduce(stagesList)(async (_, stage, idx)=>{
                ...
                if (stage.fromPermissions && stage.fromPermissions.length > 0) {
                    const fromPermissions = await utils.async.map(stage.fromPermissions, (permission)=>stagePermissionsService.register({
                            roleId: permission.role,
                            action: permission.action,
                            fromStage: stageId
                        }));
                    allPermissionIds.push(...fromPermissions.flat().map((p)=>p.id));
                }
                // Register "to" permissions
                if (stage.toPermissions && stage.toPermissions.length > 0) {
                    const toPermissions = await utils.async.map(stage.toPermissions, (permission)=>stagePermissionsService.registerTo({
                            roleId: permission.role,
                            action: permission.action,
                            toStage: stageId
                        }));
                    allPermissionIds.push(...toPermissions.flat().map((p)=>p.id));
                }
        ...
        async update (srcStage, destStage) {
            ...
            if (hasPermissionChanges) {
                ...
                allPermissionIds = [];
                const extractRoleId = (p)=>typeof p.role === 'object' ? p.role.id : p.role;
                // Re-register "from" permissions (fall back to existing "from" entries from DB)
                const fromPerms = destStage.fromPermissions ?? (srcStage?.permissions ?? []).filter((p)=>p.actionParameters?.from).map((p)=>({
                        role: extractRoleId(p),
                        action: p.action
                    }));
                if (fromPerms.length > 0) {
                    const registered = await utils.async.map(fromPerms, (permission)=>stagePermissionsService.register({
                            roleId: permission.role,
                            action: permission.action,
                            fromStage: stageId
                        }));
                    allPermissionIds.push(...registered.flat().map((p)=>p.id));
                }
                // Re-register "to" permissions (fall back to existing "to" entries from DB)
                const toPerms = destStage.toPermissions ?? (srcStage?.permissions ?? []).filter((p)=>p.actionParameters?.to).map((p)=>({
                        role: extractRoleId(p),
                        action: p.action
                    }));
                if (toPerms.length > 0) {
                    const registered = await utils.async.map(toPerms, (permission)=>stagePermissionsService.registerTo({
                            roleId: permission.role,
                            action: permission.action,
                            toStage: stageId
                        }));
                ...
                allPermissionIds = (srcStage?.permissions ?? []).map((p)=>p.id).filter(Boolean);
            }
            const stage = await strapi.db.query(workflows.STAGE_MODEL_UID).update({
                where: {
                    id: stageId
                },
                data: {
        ...
        async delete (stage) {
            ...
            const deletedStage = await strapi.db.query(workflows.STAGE_MODEL_UID).delete({
                where: {
                    id: stage.id
                }
            });
        ...
        async deleteMany (stages) {
            ...
            return strapi.db.query(workflows.STAGE_MODEL_UID).deleteMany({
                where: {
                    id: {
        ...
        count ({ workflowId } = {}) {
            ...
            return strapi.db.query(workflows.STAGE_MODEL_UID).count(opts);
        },
     ...
     */ async updateEntity (entityToUpdate, model, stageId) {
            ...
            const entity = await strapi.documents(model).update({
                ...
                data: {
                    [workflows.ENTITY_STAGE_ATTRIBUTE]: fp.pick([
                        'id'
                    ], stage)
                },
                populate: [
     ...
     */ async updateEntitiesStage (contentTypeUID, { fromStageId, toStageId }) {
            ...
            const joinTable = attributes[workflows.ENTITY_STAGE_ATTRIBUTE].joinTable;
 ...
 */ function getDiffBetweenStages(sourceStages, comparisonStages) {
    ...
    (acc, stageToCompare)=>{
        ...
        } else if (!fp.isEqual(normalizeStageForDiff(srcStage), normalizeStageForDiff(stageToCompare))) {
