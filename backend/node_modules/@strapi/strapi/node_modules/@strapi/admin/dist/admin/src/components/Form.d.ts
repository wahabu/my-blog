...
declare const useForm: <Selected, ShouldThrow extends boolean = true>(consumerName: string, selector: (value: FormContextValue<object>) => Selected, shouldThrowOnMissingContext?: ShouldThrow | undefined) => ShouldThrow extends true ? Selected : Selected | undefined;
...
type FormErrors<TFormValues extends FormValues = FormValues> = {
    [Key in keyof TFormValues]?: TFormValues[Key] extends unknown[] ? TFormValues[Key][number] extends object ? FormErrors<TFormValues[Key][number]>[] | string | string[] : string : TFormValues[Key] extends object ? FormErrors<TFormValues[Key]> : string | TranslationMessage;
};
...
declare function useField<TValue = unknown>(path: string): FieldValue<TValue | undefined>;
/**
