        ...
        async write (config, _encoding, callback) {
            await transaction?.attach(async ()=>{
                try {
                    ...
                    return callback(new providers.ProviderTransferError(`Failed to import ${chalk__default.default.yellowBright(config.type)} (${chalk__default.default.greenBright(config.value.id)}): ${error instanceof Error ? error.message : String(error)}`));
                }
                callback();
            });
        }
