        ...
        case 'uid':
            {
                return {
                    ...
                    default: yup.string().test('isValidDefaultUID', 'cannot define a default UID if the targetField is set', function isValidDefaultUID(value) {
                        const { targetField } = this.parent;
                        return !!(_.isNil(targetField) || _.isNil(value));
                    }).test('isValidDefaultRegexUID', `\${path} must match the custom regex or the default one "${UID_REGEX}"`, function isValidDefaultRegexUID(value) {
        ...
        case 'dynamiczone':
            {
                return {
                    required: validators.required,
                    components: yup.array().of(yup.string().required()).test('isArray', // eslint-disable-next-line no-template-curly-in-string -- Yup interpolation placeholder
