...
var oauthConnect;
var hasRequiredOauthConnect;
function requireOauthConnect() {
    if (hasRequiredOauthConnect) return oauthConnect;
    hasRequiredOauthConnect = 1;
    const { errors } = require$$1__default.default;
    const builtinProviderEndpoints = providers.__require();
    const oauth1$1 = oauth1.__require();
    const oauth2$1 = oauth2.__require();
    ...
    const startOAuth1Flow = async (ctx, provider, parsed, redirectUri)=>{
        const requestToken = await oauth1$1.requestToken({
    ...
    const startOAuth2Flow = (ctx, provider, parsed, redirectUri)=>{
        const state = oauth2$1.generateState();
        ctx.session.grant = {
            ...preserveGrantDynamic(ctx),
            provider: parsed.provider,
            state
        };
        let authorizeUrl = oauth2$1.buildAuthorizeUrl(provider, {
            key: provider.key,
            redirectUri,
    ...
    const handleOAuth1Callback = async (ctx, provider, callbackUrl)=>{
        ...
        const tokenResponse = await oauth1$1.accessToken({
        ...
        const payload = oauth2$1.tokensToQueryPayload(provider, tokenResponse);
        ctx.session.grant = {};
    ...
    const handleOAuth2Callback = async (ctx, provider, callbackUrl, redirectUri)=>{
        ...
        const tokenResponse = await oauth2$1.exchangeAuthorizationCode(provider, {
            key: provider.key,
    ...
    const createOAuthConnectMiddleware = ()=>{
        return async (ctx, next)=>{
            ...
            const storedProviders = await strapi.store({
                type: 'plugin',
            ...
            const callbackOverride = ctx.state.oauthConnect?.callback ?? ctx.session.grant?.dynamic?.callback;
...
exports.__require = requireOauthConnect;
