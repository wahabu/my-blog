...
import { AUDIT_LOGS_EXPORT_PART_ROWS, AUDIT_LOG_EXPORT_EVENT, AUDIT_LOGS_EXPORT_DEFAULT_MAX_ROWS, AUDIT_LOGS_EXPORT_PART_MAX_ROWS } from '../../../../../shared/utils/audit-log-export.mjs';
import { getDisplayName } from '../utils.mjs';
import { CSV_BOM, serializeCsvLine } from '../utils/csv.mjs';
import { signExportToken, verifyExportToken } from '../utils/export-token.mjs';
...
const isTrustedExportContinuation = (strapi, query)=>{
    return query.cursor !== undefined && query.until !== undefined && verifyExportToken(getExportSecret(strapi), query.token, query.until, query.filters);
};
 ...
 */ const createAuditLogsService = (strapi)=>{
    return {
        ...
        async isExportTooLarge (query) {
            const maxRows = strapi.config.get('admin.auditLogs.exportMaxRows', AUDIT_LOGS_EXPORT_DEFAULT_MAX_ROWS);
        ...
        async createExportStream (query) {
            ...
            const configuredPartSize = strapi.config.get('admin.auditLogs.exportPartRows', AUDIT_LOGS_EXPORT_PART_ROWS);
            const partSize = Math.max(1, Math.min(pageSize ?? configuredPartSize, AUDIT_LOGS_EXPORT_PART_MAX_ROWS));
            ...
            return {
                stream: Readable.from(loggedCsvChunks(), {
                    objectMode: false,
                ...
                isNewExport,
                partSize,
                exportToken: isNewExport ? signExportToken(getExportSecret(strapi), untilId, filtersQuery.filters) : token
            };
        },
