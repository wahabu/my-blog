...
const morphToMany = async (input, ctx)=>{
    ...
    if (isCount) {
        ...
        results.forEach((result)=>{
            result[attributeName] = {
                count: (joinMap[result[joinColumn.referencedColumn]] || []).length
            };
        });
        return;
    }
...
const morphToOne = async (input, ctx)=>{
    ...
    const map = {};
    const { on, ...typePopulate } = populateValue;
    const typeRestrictedTypes = on && typeof on === 'object' ? Object.keys(idsByType).filter((type)=>type in on) : Object.keys(idsByType);
    ...
    if (isCount) {
        results.forEach((result)=>{
            ...
            result[attributeName] = {
                count: id && type && allowedTypes.has(type) ? 1 : 0
            };
        });
        return;
    }
