 ...
 */ const getResetPasswordTokenTTL = ()=>{
    const configured = strapi.config.get('admin.forgotPassword.expiresIn', DEFAULT_RESET_PASSWORD_TOKEN_EXPIRES_IN);
    const seconds = token.expiresInToSeconds(configured) ?? token.expiresInToSeconds(DEFAULT_RESET_PASSWORD_TOKEN_EXPIRES_IN);
    return seconds * 1000;
};
/**
 ...
 */ const assignResetPasswordToken = async (userId)=>{
    ...
    await index.getService('user').updateById(userId, {
        resetPasswordToken,
 ...
 */ const assertResetPasswordTokenIsValid = async (user)=>{
    const { resetPasswordTokenExpiresAt } = user;
    const isExpired = !resetPasswordTokenExpiresAt || new Date(resetPasswordTokenExpiresAt).getTime() <= Date.now();
    if (isExpired) {
 ...
 */ const hashPassword = (password)=>bcrypt__default.default.hash(password, 10);
/**
 * Validate a password
 * @param password
 * @param hash
 * @returns {Promise<boolean>} is the password valid
 */ const validatePassword = (password, hash)=>bcrypt__default.default.compare(password, hash);
/**
 ...
 */ const forgotPassword = async ({ email } = {})=>{
    ...
    return strapi.plugin('email').service('email').sendTemplatedEmail({
        ...
        url,
        user: ___default.default.pick(user, [
            'email',
            'firstname',
            'lastname',
            'username'
...
exports.assertResetPasswordTokenIsValid = assertResetPasswordTokenIsValid;
exports.assignResetPasswordToken = assignResetPasswordToken;
exports.default = auth;
exports.getResetPasswordTokenTTL = getResetPasswordTokenTTL;
