import { augmentSchema, maybeWithMinMax, maybeRequired, maybeWithDefault, maybeReadonly, relations } from '@strapi/utils';
 ...
 */ const componentToSchema = (strapi, attribute)=>{
    const { writable, required, min, max, component, repeatable } = attribute;
    const componentSchema = safeSchemaCreation(strapi, component, ()=>new CoreComponentRouteValidator(strapi, component).entry);
 ...
 */ const mediaToSchema = (strapi, attribute)=>{
    ...
    const mediaSchema = safeSchemaCreation(strapi, fileSchema.uid, ()=>new CoreContentTypeRouteValidator(strapi, fileSchema.uid).document);
 ...
 */ const relationToSchema = (strapi, attribute)=>{
    if (!('target' in attribute)) {
        return z.any();
    }
    const { writable, required, target } = attribute;
    const targetSchema = safeSchemaCreation(strapi, target, ()=>new CoreContentTypeRouteValidator(strapi, target).document);
