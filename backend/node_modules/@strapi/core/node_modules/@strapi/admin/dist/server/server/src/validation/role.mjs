...
const rolesDeleteSchema = yup.object().shape({
    ids: yup.array().of(yup.strapiID()).min(1).required().test('roles-deletion-checks', 'Roles deletion checks have failed', async function checkRolesDeletion(ids) {
        try {
...
const roleDeleteSchema = yup.strapiID().required().test('no-admin-single-delete', 'Role deletion checks have failed', async function checkRoleDeletion(id) {
    try {
...
export { validateRoleCreateInput, validateRoleDeleteInput, validateRoleUpdateInput, validateRolesDeleteInput };
