...
const toOptionalStringValue = (value)=>{
    if (typeof value !== 'string' || value === '') {
        return undefined;
    }
    return value;
};
const toBooleanValue = (value)=>typeof value === 'boolean' ? value : false;
const toContentTypeKind = (value)=>value === 'singleType' ? 'singleType' : 'collectionType';
const toRecordValue = (value)=>typeof value === 'object' && value !== null ? value : {};
const toComponentDraft = (value)=>({
...
const FormModal = ()=>{
    ...
    const checkFieldNameChanges = ()=>{
        ...
        const referencedFields = contentTypeAttributes.filter((attr)=>{
            const conditionEntry = conditions.getFirstVisibleConditionEntry(attr.conditions);
    ...
    const checkFormValidity = async ()=>{
        ...
        // This is happening when the user click on the link from the left menu
            ...
            schema = forms.forms.component.schema(Object.keys(components), toStringValue(modifiedData.category), reservedNames, actionType === 'edit', components, toStringValue(modifiedData.displayName), type?.uid ?? null);
        ...
        // The data is set in the componentToCreate key
            ...
            schema = forms.forms.component.schema(Object.keys(components), toStringValue(get__default.default(modifiedData, 'componentToCreate.category', '')), reservedNames, actionType === 'edit', components, toStringValue(modifiedData.componentToCreate?.displayName));
        ...
        // We need to make sure that it is independent from the step
            ...
            if (computedAttributeType === 'relation') {
                const targetContentTypeUID = toStringValue(get__default.default(modifiedData, [
                    'target'
                ], ''));
            ...
            schema = forms.forms.attribute.schema(type, computedAttributeType ?? 'string', reservedNames, alreadyTakenTargetContentTypeAttributes, {
                modifiedData,
                initialData
            }, ctbFormsAPI);
        } else {
    ...
    const handleChange = React__namespace.useCallback(({ target: { name, value } })=>{
        ...
        if (namesThatCanResetToNullValue.includes(name) && value === '') {
            ...
            val = Array.isArray(value) ? value : typeof value === 'string' ? [
                value
            ] : [];
        } else {
            val = value;
        ...
        try {
            ...
            // We are creating a component using the component modal from the left menu
                ...
                if (isCreating) {
                    ...
                    const componentUid = createUid.createComponentUid(componentDisplayName, componentCategory);
                    ...
                    navigate({
                        pathname: `/plugins/${pluginId.pluginId}/component-categories/${componentCategory}/${componentUid}`
                    });
                    onCloseModal();
                    return;
                } else {
                    ...
                    if (type.status === 'NEW') {
                        const componentUid = createUid.createComponentUid(toStringValue(modifiedData.displayName), toStringValue(modifiedData.category));
                        updateComponentUid({
                            componentUID: targetUid,
                            newComponentUID: componentUid
                        });
                        navigate({
                            pathname: `/plugins/${pluginId.pluginId}/component-categories/${toStringValue(modifiedData.category)}/${componentUid}`
                        });
                    }
                    // Close the modal
                    onCloseModal();
                ...
                if (isDynamicZoneAttribute) {
                    ...
                    if (isCreating) {
                        ...
                        onNavigateToAddCompoToDZModal({
                            dynamicZoneTarget: toStringValue(modifiedData.name)
                        });
                    } else {
                        onCloseModal();
                    }
            ...
            // We need to prevent the component from being created if the user closes the modal at step 2 without any submission
                ...
                const componentUid = createUid.createComponentUid(draftComponent.displayName, draftComponent.category);
