...
const createBuildContext = async ({ cwd, logger, tsconfig, strapi, options = {} })=>{
   ...
   */ const strapiInstance = strapi ?? createStrapi({
    ...
    env.STRAPI_ADMIN_AUTH_COOKIE_PATH = strapiInstance.config.get('admin.auth.cookie.path') || '';
    env.STRAPI_ADMIN_AUTH_COOKIE_DOMAIN = strapiInstance.config.get('admin.auth.cookie.domain') || strapiInstance.config.get('admin.auth.domain') || '';
