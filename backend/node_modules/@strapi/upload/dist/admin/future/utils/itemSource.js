 ...
 */ const sourceFolderIdOf = (item)=>item.kind === 'folder' ? item.parentId : item.folderId;
/**
 ...
 */ const uniformSourceFolderId = (items)=>{
    if (!hasUniformSource(items)) {
        return null;
    }
    return sourceFolderIdOf(items[0]);
};
/** Whether every item in the set came from the same folder. False for an empty set. */ const hasUniformSource = (items)=>{
    if (items.length === 0) {
        return false;
    }
...
exports.hasUniformSource = hasUniformSource;
exports.sourceFolderIdOf = sourceFolderIdOf;
exports.uniformSourceFolderId = uniformSourceFolderId;
//# sourceMappingURL=itemSource.js.map
