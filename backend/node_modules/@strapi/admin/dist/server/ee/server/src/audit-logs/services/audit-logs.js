...
var exportToken = require('../utils/export-token.js');
...
const serializeCsvPayload = (payload)=>{
    if (payload === null || payload === undefined) {
        return null;
    }
    return typeof payload === 'string' ? payload : JSON.stringify(payload);
};
const parseCsvPayload = (payload)=>{
...
const getExportSecret = (strapi)=>{
    const secret = strapi.config.get('admin.auth.secret');
    if (typeof secret !== 'string' || secret.length === 0) {
        throw new Error('The audit logs export requires admin.auth.secret to be set in config/admin.js.');
    }
    return secret;
};
const isTrustedExportContinuation = (strapi, query)=>{
    return query.cursor !== undefined && query.until !== undefined && exportToken.verifyExportToken(getExportSecret(strapi), query.token, query.until, query.filters);
};
const buildExportWhere = (filtersWhere, untilId, afterId)=>({
        $and: [
            filtersWhere ?? {},
            {
                id: {
 ...
 */ const createAuditLogsService = (strapi)=>{
    return {
        ...
        async findMany (query) {
            ...
            const logs = ids.length ? await strapi.db.query('admin::audit-log').findMany({
                where: {
                    id: {
                        $in: ids
                    }
            ...
            const logsById = new Map(logs.map((log)=>[
                    log.id,
                    log
                ]));
        ...
        async isExportTooLarge (query) {
            const maxRows = strapi.config.get('admin.auditLogs.exportMaxRows', auditLogExport.AUDIT_LOGS_EXPORT_DEFAULT_MAX_ROWS);
            const { where } = strapi.get('query-params').transform('admin::audit-log', {
        ...
        isTrustedContinuation (query) {
            return isTrustedExportContinuation(strapi, query);
        },
        async createExportStream (query) {
            getExportSecret(strapi);
            const { cursor, pageSize, until, token, page: _page, sort: _sort, ...filtersQuery } = query;
            const configuredPartSize = strapi.config.get('admin.auditLogs.exportPartRows', auditLogExport.AUDIT_LOGS_EXPORT_PART_ROWS);
            const partSize = Math.max(1, Math.min(pageSize ?? configuredPartSize, auditLogExport.AUDIT_LOGS_EXPORT_PART_MAX_ROWS));
            const { where: filtersWhere } = strapi.get('query-params').transform('admin::audit-log', filtersQuery);
            const isNewExport = !isTrustedExportContinuation(strapi, query);
            if (isNewExport) {
                await strapi.eventHub.emit(auditLogExport.AUDIT_LOG_EXPORT_EVENT, {
            ...
            const getWhere = (afterId)=>buildExportWhere(filtersWhere, untilId, afterId);
            const boundary = await strapi.db.query('admin::audit-log').findMany({
                select: [
                    'id'
                ],
            ...
            async function* csvChunks() {
                if (isNewExport) {
                    yield csv.CSV_BOM + csv.serializeCsvLine(EXPORT_CSV_COLUMNS);
                }
                let lastId = cursor ?? 0;
                let remaining = partSize;
                while(remaining > 0){
                    const batchLimit = Math.min(EXPORT_BATCH_SIZE, remaining);
                    const rows = await strapi.db.query('admin::audit-log').findMany({
                        select: [
                            'id',
                            'action',
                            'date',
            ...
            async function* loggedCsvChunks() {
                try {
                    ...
                    strapi.log.error(`Audit logs export failed after the response started, the CSV part is truncated: ${error}`);
                    throw error;
                }
            }
            return {
                stream: stream.Readable.from(loggedCsvChunks(), {
                    objectMode: false,
                ...
                isNewExport,
                partSize,
                exportToken: isNewExport ? exportToken.signExportToken(getExportSecret(strapi), untilId, filtersQuery.filters) : token
            };
        },
