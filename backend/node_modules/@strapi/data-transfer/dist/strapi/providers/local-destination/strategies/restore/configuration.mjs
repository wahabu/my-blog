        ...
        async write (config, _encoding, callback) {
            await transaction?.attach(async ()=>{
                try {
                    ...
                    return callback(new ProviderTransferError(`Failed to import ${chalk.yellowBright(config.type)} (${chalk.greenBright(config.value.id)}): ${error instanceof Error ? error.message : String(error)}`));
                }
                callback();
            });
        }
