...
import { __require as __require$1 } from '../../_virtual/_node_url.mjs';

var oauth1;
var hasRequiredOauth1;
function requireOauth1() {
    if (hasRequiredOauth1) return oauth1;
    hasRequiredOauth1 = 1;
    const crypto = __require();
    const { URLSearchParams } = __require$1();
    const encode = (str)=>encodeURIComponent(str).replace(/[!'()*]/g, (c)=>`%${c.codePointAt(0).toString(16).toUpperCase()}`);
    ...
    const buildOAuth1Header = ({ method, url, params, consumerKey, clientCredential, token, tokenCredential })=>{
        ...
        const paramString = sortKeys(Object.keys(requestParameters)).map((key)=>`${encode(key)}=${encode(requestParameters[key])}`).join('&');
        ...
        const header = `OAuth ${sortKeys(Object.keys(headerParameters)).map((key)=>`${encode(key)}="${encode(headerParameters[key])}"`).join(', ')}`;
        return header;
    };
    const oauth1Request = async ({ method, url, consumerKey, clientCredential, token, tokenCredential, params = {} })=>{
    ...
    const requestToken = async ({ requestUrl, redirectUri, consumerKey, clientCredential })=>oauth1Request({
            method: 'POST',
            url: requestUrl,
            consumerKey,
            clientCredential,
    ...
    const accessToken = async ({ accessUrl, consumerKey, clientCredential, oauthToken, oauthVerifier, oauthTokenCredential })=>oauth1Request({
            method: 'POST',
            url: accessUrl,
            consumerKey,
            clientCredential,
    ...
    const twitterGet = async ({ url, accessToken, accessCredential, consumerKey, clientCredential, qs = {} })=>{
...
export { requireOauth1 as __require };
//# sourceMappingURL=oauth1.mjs.map
