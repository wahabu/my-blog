...
var contentTypes = {
    async findContentTypes (ctx) {
        ...
        try {
            await index$1.validateKind(kind);
        } catch (error) {
            return ctx.send({
                error
            }, 400);
        }
        const contentTypes = index.getService('content-types').findContentTypesByKind(kind);
    ...
    async findContentTypesSettings (ctx) {
        const { findAllContentTypes, findConfiguration } = index.getService('content-types');
    ...
    async updateContentTypeConfiguration (ctx) {
        ...
        const contentTypeService = index.getService('content-types');
        const metricsService = index.getService('metrics');
        const contentType = await contentTypeService.findContentType(uid);
        if (!contentType) {
            return ctx.notFound('contentType.notFound');
        }
        if (!index.getService('permission').canConfigureContentType({
            userAbility,
            contentType
        })) {
