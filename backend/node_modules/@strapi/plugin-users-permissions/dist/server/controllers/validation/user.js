...
function requireUser() {
    ...
    hasRequiredUser = 1;
    const { yup, validateYupSchema } = require$$1__default.default;
    ...
    const roleRelationEntrySchema = yup.object().shape({
        id: yup.strapiID(),
        documentId: yup.strapiID()
    }).test('id-or-documentId', 'Relation entry must include an id or documentId', (entry)=>!!entry && (entry.id != null || entry.documentId != null));
    const createUserBodySchema = yup.object().shape({
        email: yup.string().email().required(),
        username: yup.string().min(1).required(),
        password: yup.string().min(1).required(),
        role: yup.lazy((value)=>typeof value === 'object' ? yup.object().shape({
                connect: yup.array().of(roleRelationEntrySchema).min(1, 'Users must have a role').required()
    ...
    const updateUserBodySchema = yup.object().shape({
        ...
        role: yup.lazy((value)=>typeof value === 'object' ? yup.object().shape({
                ...
                disconnect: yup.array().of(roleRelationEntrySchema).test('CheckDisconnect', 'Cannot remove role', function test(disconnect) {
