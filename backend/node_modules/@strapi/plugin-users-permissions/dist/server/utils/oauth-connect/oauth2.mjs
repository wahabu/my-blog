...
function requireOauth2() {
    ...
    const substituteSubdomain = (url, subdomain)=>subdomain ? url.replace('[subdomain]', subdomain) : url;
    ...
    const exchangeAuthorizationCode = async (provider, { key, secret, redirectUri, code, subdomain })=>{
        ...
        if (!response.ok) {
            throw new Error(output.error_description || output.error || `Token exchange failed (${response.status})`);
        }
        return output;
    };
...
export { requireOauth2 as __require };
