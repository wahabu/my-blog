...
async function validateFile(file, config, strapi) {
    ...
    const isSvgXmlWithAllowedExt = detectedMime === 'application/xml' && fileExt === '.svg' && expectedMimeFromExt === 'image/svg+xml' && Array.isArray(allowedTypes) && allowedTypes.length > 0 && matchesMimePattern(expectedMimeFromExt, allowedTypes);
    if (detectedMime && Array.isArray(allowedTypes) && allowedTypes.length > 0 && !matchesMimePattern(detectedMime, allowedTypes) && !isZipWithAllowedExt && !isSvgXmlWithAllowedExt) {
        return {
            isValid: false,
            error: {
                code: 'MIME_TYPE_NOT_ALLOWED',
