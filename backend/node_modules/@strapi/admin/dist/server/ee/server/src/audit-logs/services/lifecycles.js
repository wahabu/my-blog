 ...
 */ const createAuditLogsLifecycleService = (strapi)=>{
   ...
   * to compile.
   */ const SYSTEM_ORIGIN_FLAGS = {
        scheduler: true
    };
    const SYSTEM_ORIGINS = Object.keys(SYSTEM_ORIGIN_FLAGS);
    const processEvent = async (name, ...args)=>{
        ...
        const systemOrigin = auditSource && SYSTEM_ORIGINS.includes(auditSource) ? auditSource : undefined;
        if (!systemOrigin && (!isUsingAdminAuth && !isMcpAdminAction || !user)) {
            return null;
        }
        const origin = systemOrigin ?? auditSource ?? 'admin-panel';
        ...
        try {
            ...
            strapi.log.error(`Failed to build the audit log payload for ${name}`, {
                error
            });
        }
        const { action: _action, date: _date, actor: _actor, origin: _origin, ...rest } = shape ?? {};
        const payload = shape ? {
            action: name,
            date,
            actor,
    ...
    const handleEvent = async (name, ...args)=>{
        try {
            ...
            strapi.log.error(`Failed to save the audit log entry for ${name}`, {
                error
            });
        }
    };
     ...
     */ registerEvent (name, transform) {
            if (eventMap[name]?.kind === 'legacy') {
                throw new Error(`Cannot register the audit log event "${name}": it is one of the built-in events, whose stored shape is frozen.`);
            }
            if (eventMap[name]) {
