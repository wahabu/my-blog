...
class MysqlSchemaInspector {
    ...
    async getBulkColumns(tableNames) {
    ...
    async getColumns(tableName) {
        const result = await this.getBulkColumns([
            tableName
        ]);
        return result.get(tableName) ?? [];
    }
    async getBulkIndexes(tableNames) {
        if (tableNames.length === 0) {
            return new Map();
        }
    ...
    async getIndexes(tableName) {
        const result = await this.getBulkIndexes([
            tableName
        ]);
        return result.get(tableName) ?? [];
    }
    async getBulkForeignKeys(tableNames) {
        if (tableNames.length === 0) {
            return new Map();
        }
        ...
        for (const row of rows){
            ...
            if (!fk) {
                ...
                if (row.referenced_column_name && !fk.referencedColumns.includes(row.referenced_column_name)) {
    ...
    async getForeignKeys(tableName) {
        const result = await this.getBulkForeignKeys([
            tableName
        ]);
        return result.get(tableName) ?? [];
    }
