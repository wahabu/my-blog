...
const createAiAdminService = ({ strapi })=>{
   ...
   */ const resolveAiContext = (errorPrefix)=>{
        ...
        if (!eeLicense) {
            try {
                const licensePath = path__default.default.join(strapi.dirs.app.root, 'license.txt');
                eeLicense = fs__default.default.readFileSync(licensePath).toString();
            } catch  {
        ...
        try {
            const response = await fetch(`${aiServerUrl}/auth/getAiJWT`, {
                ...
                headers: {
                    ...
                    'X-Request-Id': crypto__default.default.randomUUID()
                },
                body: JSON.stringify({
                    eeLicense,
                    userIdentifier,
