...
const isAllowedFieldSize = (type, size)=>{
    const { getFieldSize } = index.getService('field-sizes');
...
const getDefaultFieldSize = (attribute)=>{
    const { hasFieldSize, getFieldSize } = index.getService('field-sizes');
...
async function createDefaultLayouts(schema) {
    return {
        ...
        ...___default.default.pick(___default.default.get(schema, [
            'config',
            'layouts'
        ], {}), [
            'list',
...
function createDefaultListLayout(schema) {
    return Object.keys(schema.attributes).filter((name)=>attributes.isListable(schema, name) && name !== 'documentId').slice(0, DEFAULT_LIST_LENGTH);
}
const rowSize = (els)=>els.reduce((sum, el)=>sum + el.size, 0);
function createDefaultEditLayout(schema) {
    const keys = Object.keys(schema.attributes).filter((name)=>attributes.hasEditableAttribute(schema, name));
...
/** Synchronisation functions */ function syncLayouts(configuration, schema) {
    ...
    let cleanList = list.filter((attr)=>attributes.isListable(schema, attr));
    // TODO V5: remove editRelations
    const cleanEditRelations = editRelations.filter((attr)=>attributes.hasRelationAttribute(schema, attr));
    ...
    for (const row of edit){
        const newRow = [];
        for (const el of row){
            if (!attributes.hasEditableAttribute(schema, el.name)) continue;
            // Check if the field is a custom field with a custom size.
            // If so, use the custom size instead of the type size
            const { hasFieldSize } = index.getService('field-sizes');
    ...
    const newAttributes = ___default.default.difference(Object.keys(schema.attributes), Object.keys(configuration.metadatas));
    /** Add new attributes where they belong */ if (cleanList.length < DEFAULT_LIST_LENGTH) {
        // add newAttributes
        // only add valid listable attributes
        cleanList = ___default.default.uniq(cleanList.concat(newAttributes.filter((key)=>attributes.isListable(schema, key))).slice(0, DEFAULT_LIST_LENGTH));
    }
    // add new attributes to edit view
    const newEditAttributes = newAttributes.filter((key)=>attributes.hasEditableAttribute(schema, key));
...
exports.createDefaultLayouts = createDefaultLayouts;
exports.syncLayouts = syncLayouts;
