        ...
        const { shouldRelink } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'shouldRelink',
                message: `A project named ${chalk__default.default.cyan(existingConfig.project.displayName ? existingConfig.project.displayName : existingConfig.project.name)} is already linked to this local folder. Do you want to update the link?`,
                default: false
            }
        ]);
        if (!shouldRelink) {
...
async function getProjectsList(ctx, cloudApiService, existingConfig) {
    ...
    try {
        ...
        if (!Array.isArray(projectList)) {
            ctx.logger.log('No Cloud project found. To link a project, first create it via the Cloud dashboard (https://cloud.strapi.io).');
            return null;
        }
...
var action = (async (ctx)=>{
    ...
    try {
        ...
        logger.log(` You have successfully linked your project to ${chalk__default.default.cyan(answer.linkProject.displayName)}. You are now able to deploy your project.`);
