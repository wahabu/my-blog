   ...
   */ const getFormattedActions = async (releaseId)=>{
    ...
    return {
        async create (releaseData, { user }) {
            ...
            const { validatePendingReleasesLimit, validateUniqueNameForPendingRelease, validateScheduledAtIsLaterThanNow } = index.getService('release-validation', {
                strapi
            });
            await Promise.all([
            ...
            const release = await strapi.db.query(constants.RELEASE_MODEL_UID).create({
                data: {
            ...
            if (releaseWithCreatorFields.scheduledAt) {
                const schedulingService = index.getService('scheduling', {
                    strapi
                });
        ...
        async findOne (id, query = {}) {
            const dbQuery = strapi.get('query-params').transform(constants.RELEASE_MODEL_UID, query);
        ...
        findPage (query) {
            const dbQuery = strapi.get('query-params').transform(constants.RELEASE_MODEL_UID, query ?? {});
            return strapi.db.query(constants.RELEASE_MODEL_UID).findPage({
                ...dbQuery,
                populate: {
                    actions: {
                        count: true
        ...
        findMany (query) {
            const dbQuery = strapi.get('query-params').transform(constants.RELEASE_MODEL_UID, query ?? {});
            return strapi.db.query(constants.RELEASE_MODEL_UID).findMany({
                ...dbQuery
            });
        },
        async update (id, releaseData, { user }) {
            ...
            const { validateUniqueNameForPendingRelease, validateScheduledAtIsLaterThanNow } = index.getService('release-validation', {
                strapi
            });
            await Promise.all([
            ...
            const updatedRelease = await strapi.db.query(constants.RELEASE_MODEL_UID).update({
                where: {
                    id
                },
        ...
        async delete (releaseId) {
            ...
            await strapi.db.transaction(async ()=>{
                ...
                await strapi.db.query(constants.RELEASE_MODEL_UID).delete({
                    where: {
                        id: releaseId
                    }
                });
        ...
        async publish (releaseId) {
           ...
           */ const lockedRelease = await strapi.db?.queryBuilder(constants.RELEASE_MODEL_UID).where({
                    id: releaseId
                }).select([
                    'id',
                    'name',
                ...
                try {
                    ...
                    dispatchWebhook(constants.ALLOWED_WEBHOOK_EVENTS.RELEASES_PUBLISH, {
                        isPublished: true,
                        release
                    });
                    ...
                    await strapi.db?.queryBuilder(constants.RELEASE_MODEL_UID).where({
                        id: releaseId
                    }).update({
                        status: 'failed'
