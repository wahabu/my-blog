...
const isPathInsideRoot = (root, candidate)=>{
    const relative = path__default.default.relative(root, candidate);
    return relative === '' || !relative.startsWith(`..${path__default.default.sep}`) && !path__default.default.isAbsolute(relative);
};
...
class LocalDirectorySourceProvider {
    async bootstrap(diagnostics) {
        ...
        try {
            const stat = await fs__default.default.stat(root);
            if (!stat.isDirectory()) {
    ...
    constructor(options){
        ...
        _class_private_field_loose_base(this, _rootResolved)[_rootResolved] = path__default.default.resolve(options.directory.path);
    }
}
...
function safePath(...posixSegments) {
    const joined = path__default.default.posix.join(...posixSegments);
    const segments = joined.split(path__default.default.posix.sep).filter(Boolean);
    const resolved = path__default.default.resolve(_class_private_field_loose_base(this, _rootResolved)[_rootResolved], ...segments);
...
async function loadMetadata() {
    ...
    if (!await fs__default.default.pathExists(metadataPath)) {
    ...
    _class_private_field_loose_base(this, _metadata)[_metadata] = await fs__default.default.readJson(metadataPath);
}
async function pipeAssetsToStream(outStream, uploadsDir) {
    if (!await fs__default.default.pathExists(uploadsDir)) {
        outStream.end();
        return;
    }
    const names = (await fs__default.default.readdir(uploadsDir)).sort();
    for (const name of names){
        const absUpload = path__default.default.join(uploadsDir, name);
        const stat = await fs__default.default.stat(absUpload);
        if (stat.isFile()) {
            let metadata;
            try {
            ...
            const normalizedPath = utils.unknownPathToPosix(path__default.default.posix.join('assets', 'uploads', name));
            const asset = {
                metadata,
                filename: name,
                ...
                stream: fs__default.default.createReadStream(absUpload)
            };
...
async function readAssetMetadata(filename) {
    ...
    return fs__default.default.readJson(metadataPath);
}
async function listJsonlFiles(posixSubdir) {
    ...
    if (!await fs__default.default.pathExists(dirAbs)) {
        return [];
    }
    const names = await fs__default.default.readdir(dirAbs);
    return names.filter((n)=>n.endsWith('.jsonl')).sort().map((n)=>path__default.default.join(dirAbs, n));
}
...
async function pipeJsonlDirectoryToStream(outStream, posixSubdir) {
    ...
    for (const absPath of files){
        ...
        const stream = fs__default.default.createReadStream(absPath).pipe(streamChain.chain(transforms));
        try {
