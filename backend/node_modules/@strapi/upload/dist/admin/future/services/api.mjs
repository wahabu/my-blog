...
import { openUploadProgress, setUploadFailed, retryCancelledFiles, isUploadInFlight, appendUploadFiles, setFileError, setFileComplete, setFileUploading, setFileMetadataGenerating, setFileMetadataResult, setFileProgress } from '../store/uploadProgress.mjs';
import { createRafBatcher } from '../utils/createRafBatcher.mjs';
import { getFilenameFromUrl } from '../utils/files.mjs';
import { uploadFileViaXHR, UploadFileError, UploadAbortedError } from './uploadFileViaXHR.mjs';
 ...
 */ const poolTails = new Map();
const trackPoolRun = (uploadId, run)=>{
 ...
 */ const withTargetFolder = (file, folder)=>file.folder != null ? file : {
        ...file,
        folder: folder ?? null
    };
/**
 ...
 */ const runMergedUploadPool = async ({ entries, indices, token, uploadId, dispatch, concurrency, generateAiMetadata })=>{
    const abortController = abortControllers.get(uploadId) ?? new AbortController();
    const run = (poolTails.get(uploadId) ?? Promise.resolve(undefined)).then((previous)=>{
 ...
 */ const processSSEStream = async ({ response, dispatch, uploadId, generateAiMetadata, folderId })=>{
       ...
       */ uploadFiles: builder.mutation({
                queryFn: async ({ formData, totalFiles, concurrency = 1, generateAiMetadata }, { dispatch, getState })=>{
                    ...
                    if (isMerge) {
                        ...
                        registerUploadEntries(uploadId, mergedEntries, batch.generateAiMetadata, batch.concurrency);
