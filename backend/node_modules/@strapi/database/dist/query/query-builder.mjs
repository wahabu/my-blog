...
import { toKnexOrderByDescriptor, wrapWithDeepSort, buildStatusSortExpression, processOrderBy } from './helpers/order-by.mjs';
     ...
     */ ensurePaginationOrderStability () {
        ...
        processSelect () {
            ...
            if (this.shouldUseDistinct()) {
                ...
                const rawOrderByExpressions = state.orderBy.filter((ob)=>'rawExpression' in ob).map((ob)=>buildStatusSortExpression(db, tableName, this.alias, ob.isI18n));
                state.select = [
                    ...state.select,
        ...
        getKnexQuery () {
            ...
            switch(state.type){
                ...
                case 'min':
                    {
                        const dbColumnName = this.aliasColumn(toColumnName(meta, state.min));
                        qb.min({
                            min: dbColumnName
                        });
                        break;
