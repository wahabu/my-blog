...
const entryKey = (entry)=>`${String(entry.document_id)}:${String(entry.locale)}`;
const getCounterparts = async (trx, tableName, idsToResolve, status)=>{
    const ids = [
        ...new Set(idsToResolve.map((id)=>String(id)))
    ];
    if (ids.length === 0) {
        return {};
    }
    const drafts = await strapi.db.getConnection().select('id', 'document_id', 'locale').from(tableName).whereIn('id', ids).transacting(trx);
    if (drafts.length === 0) {
        return {};
    }
    const documentIds = [
        ...new Set(drafts.map((entry)=>entry.document_id))
    ];
    const counterpartsQuery = strapi.db.getConnection().select('id', 'document_id', 'locale').from(tableName).whereIn('document_id', documentIds).transacting(trx);
    const counterparts = status === 'published' ? await counterpartsQuery.whereNotNull('published_at') : await counterpartsQuery.whereNull('published_at');
    const counterpartByDocument = fp.keyBy(entryKey, counterparts);
 ...
 */ const load = async (uid, sourceEntries, targetStatus)=>{
    const updates = [];
    ...
    await strapi.db.transaction(async ({ trx })=>{
        for (const attribute of Object.values(dbModel.attributes)){
            ...
            const [sourceDraftRelations, targetDraftRelations] = await Promise.all([
                strapi.db.getConnection().select('*').from(joinTable.name).whereIn(sourceColumnName, sourceIds).whereNotIn(targetColumnName, sourceIds).transacting(trx),
                strapi.db.getConnection().select('*').from(joinTable.name).whereIn(targetColumnName, sourceIds).whereNotIn(sourceColumnName, sourceIds).transacting(trx)
            ]);
            const [targetCounterparts, sourceCounterparts] = await Promise.all([
                getCounterparts(trx, dbModel.tableName, sourceDraftRelations.map((relation)=>relation[targetColumnName]), targetStatus),
                getCounterparts(trx, dbModel.tableName, targetDraftRelations.map((relation)=>relation[sourceColumnName]), targetStatus)
            ]);
            if (sourceDraftRelations.length > 0) {
                const relations = sourceDraftRelations.map((relation)=>{
                    ...
                    return {
                        ...relation,
                        [targetColumnName]: targetCounterpart
                    };
                }).filter(Boolean);
            ...
            if (targetDraftRelations.length > 0) {
                const relations = targetDraftRelations.map((relation)=>{
                    ...
                    return {
                        ...relation,
                        [sourceColumnName]: sourceCounterpart
                    };
                }).filter(Boolean);
                if (relations.length > 0) {
                    updates.push({
 ...
 */ const sync = async (sourceEntries, targetEntries, relationData)=>{
    ...
    await strapi.db.transaction(async ({ trx })=>{
        for (const { joinTable, relations, remap = {
            source: true,
            target: true
        } } of relationData){
            ...
            await Promise.all(toUpdate.map((relation)=>{
                ...
                const dataToRestore = Object.fromEntries([
                    ...
                    joinTable.inverseOrderColumnName
                ].filter((columnName)=>columnName != null).filter((columnName)=>columnName in relation).map((columnName)=>[
                        columnName,
                        relation[columnName]
                    ]));
                ...
                return trx(joinTable.name).where({
