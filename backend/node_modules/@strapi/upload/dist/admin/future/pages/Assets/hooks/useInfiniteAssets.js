...
var getRelationId = require('../../../utils/getRelationId.js');
 ...
 */ const useInfiniteAssets = ({ folder = null, sort, search, filters, enabled = true } = {})=>{
    ...
    const loadedAssets = React.useMemo(()=>isChangingList ? [] : flattenPages(accumulated.pages), [
        isChangingList,
        accumulated.pages
    ]);
    ...
    const completedUploads = hooks.useTypedSelector(uploadProgress.selectCompletedUploads);
    ...
    const listAnsweredAfter = fulfilledTimeStamp !== undefined && startedTimeStamp !== undefined && // A refetch in flight leaves `fulfilledTimeStamp` on the previous response,
    // which predates this request — so only settled data counts.
    fulfilledTimeStamp > startedTimeStamp ? startedTimeStamp : undefined;
    const assets$1 = React.useMemo(()=>{
        if (!canPlaceUploads || completedUploads.length === 0) {
            return loadedAssets;
        }
        const bridging = completedUploads.filter(({ asset, completedAt })=>getRelationId.getRelationId(asset.folder) === folder && (listAnsweredAfter === undefined || completedAt > listAnsweredAfter));
        return mergeUploadedAssets.mergeUploadedAssets({
            assets: loadedAssets,
