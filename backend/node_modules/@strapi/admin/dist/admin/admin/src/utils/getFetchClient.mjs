...
import { getCookieValue, setCookie, AUTH_COOKIE_NAME } from './cookies.mjs';
import { decodeAccessTokenExpiry } from './jwt.mjs';
...
const postAccessToken = (backendURL)=>fetch(`${backendURL}/admin/access-token`, {
        method: 'POST',
        credentials: 'include',
        headers: {
...
const adoptSharedAccessToken = (token)=>{
    if (onTokenUpdate) {
        onTokenUpdate(token);
    }
    return token;
 ...
 * storage counts.
 */ const readValidAccessTokenFromSharedStorage = (staleToken)=>{
...
const waitForSharedAccessToken = async (staleToken)=>{
...
const executeRefreshAccessToken = async (backendURL, tokenBeforeRefresh, { allowHttpRetry })=>{
 ...
 */ const refreshAccessToken = async (tokenUsedByFailedRequest)=>{
    const backendURL = window.strapi.backendURL;
    const tokenBeforeRefresh = tokenUsedByFailedRequest ?? getToken();
    try {
        if (typeof navigator !== 'undefined' && navigator.locks?.request) {
            return await navigator.locks.request(ADMIN_REFRESH_LOCK, ()=>executeRefreshAccessToken(backendURL, tokenBeforeRefresh, {
 ...
 * @internal Exported for testing purposes
 */ const attemptTokenRefresh = async (tokenUsedByFailedRequest)=>{
    if (!refreshPromise) {
 ...
 */ const getFetchClient = (defaultOptions = {})=>{
   ...
   */ const withTokenRefresh = async (url, executeRequest, getRequestToken)=>{
        try {
    ...
    const fetchClient = {
        get: async (url, options)=>{
            ...
            const executeRequest = async ()=>{
                ...
                const defaultHeaders = responseType === 'json' ? getDefaultHeaders(requestToken) : {
                    Authorization
                };
...
export { ADMIN_REFRESH_LOCK, FetchError, attemptTokenRefresh, getFetchClient, isFetchError, refreshAccessToken, setOnSessionExpired, setOnTokenUpdate, storeToken, triggerSessionExpired };
